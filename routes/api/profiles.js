...
router.get("/github/:username", (req, res) => {
	const githubUrl = `https://api.github.com/users/${req.params.username}/repos`;
	const githubParams = {
	...
	return axios
		.get(githubUrl, {
         params: githubParams,
         // removes session auth token from being submitted to github api
			headers: {
				Authorization: null,
			},
      })
		.then(repoData => res.json(repoData.data))
		.catch(err => res.status(404).json({ noGithub: "Github user not found." }));
});
